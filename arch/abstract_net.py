...
class AbstractNetwork:
  ...
  def build_graph(self, train_stage, data_format="NHWC"):
  ...
  def train(self, path=None):
    ...
    self.batch_size_gpu = self.batch_size
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
      ...
      with tf.name_scope("train_model"):
        with tf.variable_scope("model"):
          for i in range(self.n_gpus):
            with tf.device(self.assign_to_device('/cpu:%d' % i)):
      ...
      for self.step in range(max_steps):
        ...
        # print(self.dataset.train, self.img_pl_list, self.lbls_pl_list, self.batch_size_gpu)
        start_time = time.time()
        ...
        if self.step % self.TRAIN["summary_freq"] == 0:
          ...
          # print('*'*80, '\n', feed_dict)
          # print('*'*80, '\n', 'Confusion Matrix: \n\n', self.sess.run(self.confusion_matrix))
